...
<div class="vue-cropper" ref="cropper" @mouseover="scaleImg" @mouseout="cancleScale">
  <div class="cropper-loading" v-if="isLoadingImg">
    <div class="topBar-more"><i class="iconfont icon-icon-loading"></i></div>
  </div>
        ...
        if (this.canChangeX) {
          if (this.changeCropTypeX === 1) {
            if (this.cropOldW - fw > 0) {
              this.cropW = Math.max(Math.min(this.w - this.cropChangeX - fw <= this.w ? this.cropOldW - fw : this.cropOldW + this.cropChangeX, this.maxW), this.minW)
              this.cropOffsertX = this.w - this.cropChangeX - fw <= this.w ? this.cropChangeX + fw : 0
            } else {
              this.cropW = Math.max(Math.min(Math.abs(fw) + this.cropChangeX <= this.w ? Math.abs(fw) - this.cropOldW : this.w - this.cropOldW - this.cropChangeX, this.maxW), this.minW)
              this.cropOffsertX = this.cropChangeX + this.cropOldW
            }
          } else if (this.changeCropTypeX === 2) {
            if (this.cropOldW + fw > 0) {
              this.cropW = Math.max(Math.min(this.cropOldW + fw + this.cropOffsertX <= this.w ? this.cropOldW + fw : this.w - this.cropOffsertX, this.maxW), this.minW)
              this.cropOffsertX = this.cropChangeX
            } else {
              this.cropW = Math.max(Math.min((this.w - this.cropChangeX + Math.abs(fw + this.cropOldW)) <= this.w ? Math.abs(fw + this.cropOldW) : this.cropChangeX, this.maxW), this.minW)
...
.cropper-loading {
  ...
  i {
    display: block;
    font-size: 80px;
    animation: cropLoading 2s infinite linear;
    color: #1e1e20;
  }
  @keyframes cropLoading {
    from {
      transform: rotate(0);
    }
    to {
